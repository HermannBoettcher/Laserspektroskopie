  ...
  \chapter{Versuch}
    ...
    \label{sec:linienbreite}
      ...
      \label{subsec:linienbreite-laser}
        ...
        \Cref{fig:linienbreite-plot} zeigt die Daten der dopplerfreien Messung zur Bestimmung der Linienbreite des Lasers. Ch1 ist das Signal vor dem \textit{Fabry-Perot-Etalon}, ch2 das hinter dem Etalon und ch3 ist die Aufzeichnung der Spannungsmodulation des Netzteils des Diodenlasers. Zunächst wird ch2 betrachtet.

        Es erscheinen fünf Maxima. Während die Spannung monoton steigend oder fallend durchgefahren wird ändert sich die Frequenz des Laserlichts gleichgerichtet. Damit repräsentiert der Abstand zwischen zwei Maxima über einem monton steigenden/ fallenden Signal von ch3 $\Delta t_\mathrm{FPE}$ gerade den freien Spektralbereich des \textit{Fabry-Perot-Etalons} $Delta f_\mathrm{FPE}$ (vgl. \cref{eq:freier-spektralbereich}).

        Damit ist die \textit{Linienbreite} des Lasers ist gegeben durch den Zusammenhang
        \begin{equation}
        ...
        \begin{align*}
          \Delta f_\mathrm{Laser}^\mathrm{aufsteigend, links}=\SI{10,81(59)}{\mega\hertz} \\
          \Delta f_\mathrm{Laser}^\mathrm{aufsteigend, rechts}=\SI{8,30(32)}{\mega\hertz} \\
          \Delta f_\mathrm{Laser}^\mathrm{absteigend, links}=\SI{11,59(41)}{\mega\hertz} \\
          \Delta f_\mathrm{Laser}^\mathrm{absteigend, rechts}=\SI{13,85(58)}{\mega\hertz}.
        \end{align*}
        Mitteln der zusammengehörenden Werte liefert dann
        \begin{align*}
          \Delta f_\mathrm{Laser}^\mathrm{aufsteigend}=\SI{9,56(34)}{\mega\hertz} \\
          \Delta f_\mathrm{Laser}^\mathrm{absteigend}=\SI{12,72(36)}{\mega\hertz} ,
        \end{align*}
        und noch einmal mitteln führt zu
        \begin{equation*}
          \Delta f_\mathrm{Laser}=\SI{11.14(25)}{\mega\hertz}.
        \end{equation*}
